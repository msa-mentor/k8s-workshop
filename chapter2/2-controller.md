...
이 절에서 실습해 볼 Controller는 이런 이상상황에서도 Pod의 재기동등 관리할 수 있는 쿠버네티스의 리소스이다. 
...
### 1.1 Controller's Reconciliation loop (조정)

![](img/2-2.png)

RC는 Label Selector와 일치하는 포드 숫자와(desired) 현재 실행(current)를 비교해서, 포드 숫자를 조절한다. 

### 1.2 Replication Controller의 이용하는 이점
...
#### 1.3.1 Replication Controller 생성
...
#### 1.3.2 Replication Controller 실행확인
